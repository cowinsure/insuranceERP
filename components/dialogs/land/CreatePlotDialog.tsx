...
} from "@/components/model/Land/LandSubmissionModel";
...
export function CreatePlotDialog({
  ...
  const [suitabilityReasons, setSuitabilityReasons] = useState<
    LandSuitabilityRemark[]
  >([]);
  const [nonSuitabilityReasons, setNonSuitabilityReasons] = useState<
    LandSuitabilityRemark[]
  >([]);
  ...
  const SUITABILITY_OPTIONS = useMemo(
    () =>
      landSuitabilities
        ...
        .map((s) => ({
          id: String(s.land_suitability_id),
          text: s.land_suitability_name,
        })),
    [landSuitabilities]
  );
  ...
  useEffect(() => {
    ...
    // ⛔ stop if value is not a valid number yet
    if (!Number.isFinite(base) || base <= 0) {
      setMeasureSENE("Randomly generated length");
      return;
    }
    ...
    const randomLength = generateRandomNumber(base) + 7;
    setMeasureSENE(randomLength.toString());
  }, [measureSWSE]);
  ...
  useEffect(() => {
    ...
    // ⛔ Guard: no calculation until valid input exists
    if (!Number.isFinite(base) || base <= 0) {
      setMeasureNWNE("Randomly generated width");
      return;
    }

    const range = Math.max(1, Math.floor(base * 0.3));
    const randomWidth = Math.floor(Math.random() * range) + 1 + 7;
  ...
  useEffect(() => {
    ...
    const fetchSuitability = async () => {
      setLsLoading(true);
      try {
        const resp = await get(`lams/land-suitability-service/`, {
  ...
  const generatePlot = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://cropploting.dev.insurecow.com/landmap/generate/",
        {
          ...
          headers: {
            ...
            Authorization:
              "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJTb21lLXRlc3QtdXNlci0xMjM0NSIsInJvbGUiOiJBZG1pbl9UZXN0IiwiZXhwIjo4ODE1ODAyMjI4N30.EKD2bGCZ4KzaVrxjtf2wWE9XYIzbS_V-VKGPQIHsuyY",
          },
      ...
      const area =
        typeof data.area === "number" ? `${data.area.toFixed(2)} acres` : "N/A";
      ...
      const apiPlotData: PlotData = {
        ...
        swMark: sw_mark_raw
          ? {
              lat: sw_mark_raw.latitude.toString(),
              lng: sw_mark_raw.longitude.toString(),
            }
          : null,
        nCorner: n_corner_raw
          ? {
              lat: n_corner_raw.latitude.toString(),
              lng: n_corner_raw.longitude.toString(),
            }
          : null,
        eCorner: e_corner_raw
          ? {
              lat: e_corner_raw.latitude.toString(),
              lng: e_corner_raw.longitude.toString(),
            }
          : null,
        nMark: n_mark_raw
          ? {
              lat: n_mark_raw.latitude.toString(),
              lng: n_mark_raw.longitude.toString(),
            }
          : null,
        eMark: e_mark_raw
          ? {
              lat: e_mark_raw.latitude.toString(),
              lng: e_mark_raw.longitude.toString(),
            }
          : null,
        ...
        intersection: intersection_raw
          ? {
              lat: intersection_raw.latitude.toString(),
              lng: intersection_raw.longitude.toString(),
            }
          : null,
        imageUrl,
        area,
  ...
  const savePlot = async () => {
    if (apiPayload) {
      ...
      toast({
        title: t("plot_saved"),
        description: `${plotData?.plotName ?? ""} ${t("added_to_plots")}`,
      });

      //plot coordinate mapping
      const coordinates: LandCoordinatePoint[] =
        plotData?.plotCoordinates.map((coord) => ({
      ...
      const innerCoordinates: LandCoordinatePoint[] =
        plotData?.innerCoordinates.map((coord) => ({
      ...
      //inner land  coordinate mapping
      const landCoordinates: LandCoordinatePoint[] =
        plotData?.landCoordinates.map((coord) => ({
      ...
      const sw_mark = plotData?.swMark
        ? { latitude: plotData.swMark.lat, longitude: plotData.swMark.lng }
        : null;
      const n_corner = plotData?.nCorner
        ? { latitude: plotData.nCorner.lat, longitude: plotData.nCorner.lng }
        : null;
      const e_corner = plotData?.eCorner
        ? { latitude: plotData.eCorner.lat, longitude: plotData.eCorner.lng }
        : null;
      const n_mark = plotData?.nMark
        ? { latitude: plotData.nMark.lat, longitude: plotData.nMark.lng }
        : null;
      const e_mark = plotData?.eMark
        ? { latitude: plotData.eMark.lat, longitude: plotData.eMark.lng }
        : null;
      const intersection = plotData?.intersection
        ? {
            latitude: plotData.intersection.lat,
            longitude: plotData.intersection.lng,
          }
        : null;
      ...
      try {
        const resp = await landSubmissionAPi(
          `lams/land-info-service/`,
          landSubmissionModel
        );
  ...
  function sortPolygonCoords(coords: Coordinate[]): Coordinate[] {
    ...
    return [...coords].sort((a, b) => {
      const angleA = Math.atan2(
        parseFloat(a.lat) - centroid.lat,
        parseFloat(a.lng) - centroid.lng
      );
      const angleB = Math.atan2(
        parseFloat(b.lat) - centroid.lat,
        parseFloat(b.lng) - centroid.lng
      );
                        ...
                        onClick={(e) => {
                          ...
                          if (newLat && newLng) {
                            ...
                            if (
                              !coordinates.some(
                                (coord) =>
                                  coord.lat === newLat && coord.lng === newLng
                              )
                            ) {
                              setCoordinates([
                                ...coordinates,
                ...
                <CardContent>
                  <GoogleMap
                    ...
                    center={
                      userLocation
                        ? userLocation
                        : plotData.plotCoordinates &&
                          plotData.plotCoordinates.length > 0
                        ? {
                  ...
                  >
                    ...
                    {plotData.plotCoordinates &&
                      plotData.plotCoordinates.length > 2 && (
                        <Polygon
                          path={sortPolygonCoords(plotData.plotCoordinates).map(
                            (coord) => ({
                              lat: parseFloat(coord.lat),
                              lng: parseFloat(coord.lng),
                            })
                    ...
                    {plotData.innerCoordinates &&
                      plotData.innerCoordinates.length > 2 && (
                        <Polygon
                          path={sortPolygonCoords(
                            plotData.innerCoordinates
                          ).map((coord) => ({
                            lat: parseFloat(coord.lat),
                            lng: parseFloat(coord.lng),
                    ...
                    {plotData.landCoordinates &&
                      plotData.landCoordinates.length > 2 && (
                        <Polygon
                          path={sortPolygonCoords(plotData.landCoordinates).map(
                            (coord) => ({
                              lat: parseFloat(coord.lat),
                              lng: parseFloat(coord.lng),
                            })
                    ...
                    {plotData.swMark && (
                      <Marker
                        key={`sw_mark`}
                        position={{
                          lat: parseFloat(plotData.swMark.lat),
                          lng: parseFloat(plotData.swMark.lng),
                        }}
                    ...
                    {plotData.nCorner && (
                      <Marker
                        key={`n_corner`}
                        position={{
                          lat: parseFloat(plotData.nCorner.lat),
                          lng: parseFloat(plotData.nCorner.lng),
                        }}
                    ...
                    {plotData.eCorner && (
                      <Marker
                        key={`e_corner`}
                        position={{
                          lat: parseFloat(plotData.eCorner.lat),
                          lng: parseFloat(plotData.eCorner.lng),
                        }}
                    ...
                    {plotData.nMark && (
                      <Marker
                        key={`n_mark`}
                        position={{
                          lat: parseFloat(plotData.nMark.lat),
                          lng: parseFloat(plotData.nMark.lng),
                        }}
                    ...
                    {plotData.eMark && (
                      <Marker
                        key={`e_mark`}
                        position={{
                          lat: parseFloat(plotData.eMark.lat),
                          lng: parseFloat(plotData.eMark.lng),
                        }}
                    ...
                    {plotData.intersection && (
                      <Marker
                        key={`intersection`}
                        position={{
                          lat: parseFloat(plotData.intersection.lat),
                          lng: parseFloat(plotData.intersection.lng),
                        }}
