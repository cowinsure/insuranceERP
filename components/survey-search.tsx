...
export function SurveySearch({
  placeholder = "Search surveys by Farmer Name, Location or Plot ID",
  data,
  setFilteredData,
  searchKeys = ["farmer_name", "location", "plot_id"],
}: Props) {
  ...
  useEffect(() => {
    // when dataset changes reset filtered state
  ...
  return (
    <div className="flex flex-col space-y-2 border bg-white py-6 px-5 rounded-lg">
      ...
      {query && (
        <>
          {noResults ? (
            <p className="text-sm text-red-500 animate-fadeIn">No results found</p>
          ) : (
